...
BASE_IMAGE := "us-central1-docker.pkg.dev/your-gcp-project/deberta-api/deberta-api:model-base"

# Build and push the model base image (run once, or when the model changes)
build-base:
    docker buildx build --platform linux/amd64 \
        -t {{BASE_IMAGE}} \
        -f services/deberta-api/Dockerfile.base \
        --push .

# Build Docker image for linux/amd64 (required for GCP from Apple Silicon)
docker-build:
    docker buildx build --platform linux/amd64 \
        --build-arg MODEL_BASE={{BASE_IMAGE}} \
        --cache-from type=registry,ref={{IMAGE}} \
        --cache-to type=inline \
        -t {{IMAGE}} -f services/deberta-api/Dockerfile .
